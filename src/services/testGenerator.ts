...
export function generateMathTest(parameters: TestParametersData): TestQuestion[] {
  ...
  const numbers = generateNumbers(parameters);
...
const generateNumbers = (parameters: TestParametersData): number[] => {
...
const generateValidQuestion = (
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    // Division is generated by first creating a multiplication question and then finding the divisor
    if (operator === ':') {
      ...
      if (
        isValidQuestion(firstNumber, secondNumber, '·') && 
        !isDuplicateQuestion(existingQuestions, firstNumber, secondNumber, '·')
      ) {
        return { firstNumber: result, secondNumber };
      }
    } else {
      const firstNumber = getRandomElement(numbers);
      const secondNumber = getRandomElement(numbers);
